...
load = Image.open(r'treen.jpg') #[2]
loadx= Image.open(r'plantmoney.jpg')
image1 = ImageTk.PhotoImage(loadx)
...
load4 = Image.open(r'piggycalcu.jpg')
image_a = ImageTk.PhotoImage(load4)
load_a =Image.open(r'fdimage.jpg')
image_b = ImageTk.PhotoImage(load_a)
...
def display_page(b=True):
    ...
    Label(root, text="TRUSTY BANK", font=("Times new Roman", 46, "bold", "underline"), bg="white", fg="green").place(x=500, y=80) #[4]
    ...
    def sign_up():
        ...
        def con():
            ...
            if (name != "") and (phone_no != "") and (g_mail != "") and (len(phone_no)==10) and phone_no.isdigit() and (g_mail.endswith("@gmail.com") or g_mail.endswith("@GMAIL.COM")):
                for widget in root.winfo_children():
                    widget.destroy()
                date_time_display()
                Label(root, image=img, bg="white").place(x=0, y=0)
                ...
                message = 'Subject: {}\n\n{}'.format("OTP for sign up",otp_gen )
                server = smtplib.SMTP_SSL("smtp.gmail.com", 465) #[5]
                server.login("trustybank1111@gmail.com", "#Arch1@#")
                server.sendmail("trustybank1111@gmail.com", g_mail,message)
                server.quit()
                ...
                def deposit_amount():
                    ...
                    def Insert_f():
                        ...
                        if bal_amt.isdigit() and int(bal_amt) >= 2500:
                            ...
                            b9=Button(root,text="Back",command=partial(display_page,b=False),bg="green",font=("Calibre",15,"bold"),fg="white")
                    ...
                    h_b1(b7)
                    b8=Button(root,text="Back",command=sign_up,font=("Calibre",15,"bold"),fg="white",bg="green")
                    ...
                    def check_password():
                        ...
                        if (len(password)>=8) and l>0 and u>0 and s>0 and d>0:
                            if password == confo_password and (password != "") and (confo_password != ""):
                                ...
                                Label(root,text="*Passwords do not match",font=("Times new roman",15),bg="white",fg="red").place(x=600,y=450)
                ...
                h_b1(b2)
                abc =Button(root,text="Back",command=sign_up,bg="green",font=("Calibre",15,"bold"),fg="white")
                abc.place(x=672,y=430)
                h_b1(abc)
            else:
    ...
    def login():
        ...
        def options():
            ...
            def rd(): #[8],[9]
                ...
                panel.place(x=230,y=280)
                amt_rd2 = StringVar()
                date_time_display()
                Label(root, text="For any queries,send an email to trustybank1111@gmail.com", font=("times new roman", 9,),bg="white").place(x=700, y=678)
                Label(root, text="RD CALCULATOR", font=("Times new Roman", 46, "bold", "underline"),fg="green",bg="white").place(x=100, y=50)
                Label(root, text="Select tenure in months",font=("Times new roman",20),bg="white").place(x=125,y=200)
                Label(root, text="Enter amount",font=("Times new roman",20),bg="white").place(x=125,y=313)
                Label(root, text="Are you a senior citizen?",font=("Times new roman",20),bg="white").place(x=125,y=450)
                e1 = Scale(root, from_=3, to=120, orient="horizontal", background='lightyellow', fg='black',
                           troughcolor='sandybrown', activebackground='sienna', length=280, font=("arial", 13),
                           width=10) #[10],[11],[12]
                e1.place(x=125, y=250)
                e2 = Entry(root, textvariable=amt_rd2,font=("arial",19),bg="lightyellow")
                e2.place(x=125,y=360)
                Label(root,text="*Investment tenure can range from 3 to 120 months",font=("Times new roman",9),bg="white").place(x=125,y=291)
                Label(root,text="*Minimum monthly deposit is ₹ 100",font=("Times new roman",9),bg="white").place(x=125,y=395)
                def senior_yes():
                    duration_rd=int(e1.get())
                    monthly_amt_rd=str(amt_rd2.get())
                    monthly_amt_rd1=str(amt_rd2.get())
                    num = monthly_amt_rd1.replace('.','1')
                    if monthly_amt_rd != "" and num.isdigit() and float(monthly_amt_rd) >= 100.0:
                        for widget in root.winfo_children():
                            widget.destroy()
                        panel = Label(root, image=image_a,bg="white")
                        panel.place(x=230,y=280)
                        ...
                        n = duration_rd/3
                        a = 1.0 + ((roi_rd/100)/n)
                        for i in range(1,(duration_rd+1)):
                            t = (n*i)/12
                            final_amt_rd += monthly_amt_rd * (a**t)
                        final_amt_rd = round(final_amt_rd,2)
                        interest_rd = final_amt_rd - (monthly_amt_rd*duration_rd)
                        Label(root, text="Amount on maturity : ₹ "+str(final_amt_rd) ,font=("Times new roman",20),bg="white",fg="dark green").place(x=125,y=300)
                        Label(root, text="Interest obtained : ₹ "+str(round(interest_rd,2)),font=("Times new roman",20),bg="white",fg="dark green").place(x=125,y=350)
                        Label(root, text="Invested amount : ₹ "+str(round(monthly_amt_rd*duration_rd,2)),font=("Times new roman",20),bg="white",fg="dark green").place(x=125,y=400)
                        b4=Button(root, text="Back", command=rd, bg="green",font=("Calibre",15,'bold'),fg='white')
                        b4.place(x=125,y=450)
                        h_b1(b4)
                    else:
                        Label(root,text="*Invalid entry", font=("Times New Roman", 15), bg="white",fg="red").place(x=125,y=545)
                        amt_rd2.set("")
                def senior_no():
                    duration_rd=int(e1.get())
                    monthly_amt_rd=str(amt_rd2.get())
                    ...
                    if monthly_amt_rd != "" and num.isdigit() and float(monthly_amt_rd) >= 100.0:
                        ...
                        Label(root, text="Interest obtained : ₹ " + str(round(interest_rd, 2)),font=("Times new roman", 20), bg="white", fg="dark green").place(x=125, y=350)
                        Label(root, text="Invested amount : ₹ " + str(round(monthly_amt_rd * duration_rd, 2)),font=("Times new roman", 20), bg="white", fg="dark green").place(x=125, y=400)
                        b5 = Button(root, text="Back", command=rd, bg="green", font=("Calibre", 15, 'bold'), fg='white')
                        b5.place(x=125, y=450)
                        h_b1(b5)
                    else:
                        Label(root,text="*Invalid entry", font=("Times New Roman", 15), bg="white",fg="red").place(x=125,y=545)
                        amt_rd2.set("")
                b1=Button(root, text="Yes", command=senior_yes,bg="green",font=("Calibre",15,"bold"),fg="white")
                b1.place(x=125,y=500)
                h_b1(b1)
                b2=Button(root, text=" No ", command=senior_no,bg="green",font=("Calibre",15,"bold"),fg="white")
                b2.place(x=185,y=500)
                h_b1(b2)
                b3=Button(root, text="Back", command=options,bg="green",font=("Calibre",15,"bold"),fg="white")
                b3.place(x=125,y=580)
                h_b1(b3)
            def fd(): #[8][9]
                global image_b
                for widget in root.winfo_children():
                    widget.destroy()
                panel = Label(root, image=image_b,bg="white")
                panel.place(x=380,y=200)
                amt_fd2 = StringVar()
                date_time_display()
                Label(root, text="For any queries,send an email to trustybank1111@gmail.com", font=("times new roman", 9,),bg="white").place(x=700, y=678)
                Label(root, text="FD CALCULATOR", font=("Times new Roman", 46, "bold", "underline"), fg="green",bg="white").place(x=100, y=50)
                Label(root, text="Select tenure in months",font=("Times new roman",20),bg="white").place(x=125,y=200)
                Label(root, text="*Investment tenure can range from 1 to 120 months", font=("Times new roman", 9),bg="white").place(x=125, y=291)
                Label(root, text="Enter amount",font=("Times new roman",20),bg="white").place(x=125,y=313)
                Label(root, text="*Minimum monthly deposit is ₹ 10000", font=("Times new roman", 9), bg="white").place(x=125, y=395)
                Label(root, text="Are you a senior citizen?",font=("Times new roman",20),bg="white").place(x=125,y=450)
                e1 = Scale(root, from_=1, to=120, orient="horizontal", background='lightyellow', fg='black',
                           troughcolor='sandybrown', activebackground='sienna', length=280, font=("arial", 13),
                           width=10) #[10],[11],[12]
                e1.place(x=125, y=250)
                e2 = Entry(root, textvariable=amt_fd2,font=("arial",19),bg="lightyellow")
                e2.place(x=125, y=360)
                def senior_yes():
                    amt_fd = str(amt_fd2.get())
                    duration_fd = int(e1.get())
                    amt_fd1 = str(amt_fd2.get())
                    num = amt_fd1.replace('.', '1')
                    if amt_fd != "" and num.isdigit() and float(amt_fd) >= 10000.0:
                        for widget in root.winfo_children():
                            widget.destroy()
                        panel = Label(root, image=image_b,bg="white")
                        panel.place(x=380,y=200)
                        ...
                        if amt_fd < 20000000:
                            ...
                            if duration_fd <= 1:
                                ...
                                roi_fd = 4.7
                        duration_fd /= 12
                        interest_fd = (duration_fd * amt_fd * roi_fd )/ 100
                        final_amt_fd = amt_fd + interest_fd
                        final_amt_fd=round(final_amt_fd,2)
                        Label(root, text="Amount on maturity : ₹ " + str(final_amt_fd), font=("Times new roman", 20),
                              bg="white", fg="dark green").place(x=125, y=300)
                        Label(root, text="Interest obtained : ₹ " + str(round(interest_fd, 2)),
                              font=("Times new roman", 20), bg="white", fg="dark green").place(x=125, y=350)
                        Label(root, text="Invested amount : ₹ " + str(round(amt_fd, 2)),
                              font=("Times new roman", 20), bg="white", fg="dark green").place(x=125, y=400)
                        b4 = Button(root, text="Back", command=fd, bg="green", font=("Calibre", 15, 'bold'), fg='white')
                        b4.place(x=125, y=450)
                        h_b1(b4)
                    else:
                        Label(root, text="*Invalid entry", font=("Times New Roman", 15), bg="white", fg="red").place(x=125, y=545)
                    ...
                    if str(payee_name.get()) != "" and str(amt_dd2.get()) != "" and str(amt_dd2.get()).isdigit():
                        ...
                        def print_dd():
                            ...
                            if amt_dd>amt:
                                ...
                                amt_dd2.get()) + "\nDD number: " + dd_no
                                message = 'Subject: {}\n\n{}'.format("Electronic-DD Trusty Bank", dd)
                                server = smtplib.SMTP_SSL("smtp.gmail.com", 465)
                                server.login("trustybank1111@gmail.com", "#Arch1@#")
                                server.sendmail("trustybank1111@gmail.com", e_mail,message)
                                server.quit()
                                f1 = open('dd.txt', 'a')
                ...
                def choose():
                    ...
                    def home_loan():
                        ...
                        if n3.isdigit() and amt3.isdigit() and credit_score3.isdigit():
                            if int(credit_score3) >= 0 and int(credit_score3) <= 850 and int(n3) != 0 and int(amt3) != 0:
                                def saleried_home():
                ...
                def calc(roi,pic,b):
                    ...
                    if roi != -1:
                        ...
                        if pic == 1:
                            ...
                            insert = "Rate of interest : " + str(roi) + " % p.a "
                            insert2 = "Monthly installment : ₹ " + str(monthly_amt)[0:x + 3:]
